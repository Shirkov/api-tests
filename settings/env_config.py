...
def run_tests(env: str):
    if env == 'prod':
        sys.exit(
            pytest.main(["-v",  "-s", "--reruns", "3", "--reruns-delay", "2", "--alluredir=./allureresult"],
                        plugins=[MyPlugin()]))
    elif env == 'dev':
        sys.exit(
            pytest.main(["-v", "-s", "--reruns", "3", "--reruns-delay", "2", "--alluredir=./allureresult"],
...
class ConfigLoader:
    ...
    def set_env(self):
        ...
        os.environ["ENV"] = args.get("env") or "dev"
    ...
    def merge_config(self):
        ...
        if os.getenv("CMS_SYS_LOGIN"):
            self.environments.cms.cms_sys_login = os.getenv("CMS_SYS_LOGIN")

        if os.getenv("CMS_SYS_PASSWORD"):
            self.environments.cms.cms_sys_password = os.getenv("CMS_SYS_PASSWORD")
